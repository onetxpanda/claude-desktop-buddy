...
bool characterInit(const char* name) {
  if (!LittleFS.begin(true)) {
    // begin(true) formats on first failure; if it still fails, FS is broken
    if (!LittleFS.open("/")) {
      Serial.println("[char] LittleFS mount/format failed");
      return false;
    }
  }
...
void characterRenderTo(M5GFX* tgt, int cx, int cy) {
  ...
  lgfx::LGFXBase* prevT = _tgt; bool prevP = peekMode; int px = gifX, py = gifY;
...
void characterTick() {
  ...
  if (textMode) {
    ...
    canvas.fillRect(0, cy - 14, canvas.width(), 28, pal.bg);

    const char* line = ts.frames[textFrame];
    int len = strlen(line);
    int tw = len * 12;                                    // size-2 glyph width
    canvas.setTextColor(pal.body, pal.bg);
    canvas.setTextSize(2);
    canvas.setCursor((canvas.width() - tw) / 2, cy - 8);
    canvas.print(line);
